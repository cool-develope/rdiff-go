...
func New(windowSize uint32) *Adler32 {
	return &Adler32{
		a:       1,
		b:       0,
		n:       0,
...
func (d *Adler32) Write(data []byte) (int, error) {
	...
	d.vanilla.Reset()
	d.vanilla.Write(d.window) //nolint
	s := d.vanilla.Sum32()
	d.a, d.b = s&0xffff, s>>16 //nolint
...
func (d *Adler32) Sum(b []byte) []byte {
	v := d.Sum32()
	return append(b, byte(v>>24), byte(v>>16), byte(v>>8), byte(v)) //nolint
}
